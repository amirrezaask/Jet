# Yaade — Senior PM UX Review (2026-07)
...
- `data-yaade-overlay="prompt"` → 24rem (goto-line class: one-shot input).
- `data-yaade-overlay="picker"` → 32rem (buffer, quick-open, palette, outline, workspace picker).
- `data-yaade-overlay="wide"` → 42rem (project switcher, future search).
...
2. **Overlay width chaos.**
   ...
   *Fix:* three CSS tokens + `data-yaade-overlay` attribute, per Section B.
   *Effort:* small.
