...
describe("shell-env", () => {
  ...
  it("enrichProcessPath prepends missing user bins on partial GUI PATH", () => {
    ...
    const previousForce = process.env.GHARARGAH_SHELL_ENV_FORCE
    try {
      delete process.env.GHARARGAH_SHELL_ENV_FORCE
      ...
      if (previousForce === undefined) delete process.env.GHARARGAH_SHELL_ENV_FORCE
      else process.env.GHARARGAH_SHELL_ENV_FORCE = previousForce
    }
  })
})
