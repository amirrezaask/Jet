...
import { JetPanelTree, buildTabsView } from "@jet/workspace"
  ...
  return {
    ...
    tabRegistry: {
      kindFor(id: string) {
        ...
        if (id.startsWith("jet:terminal:")) return "terminal"
        if (id.startsWith("jet:agent-chat:")) return "agent-chat"
        if (id === "jet:search") return "search"
        return undefined
      },
    },
...
describe("resolveContextWorkspaceFolder", () => {
  it("uses last context when search tab is focused", () => {
    const { tree, editorPanel } = JetPanelTree.editorOnlyLayout()
    tree.setView(editorPanel, buildTabsView("jet:search", ["jet:search"]))
    const workspace = mockWorkspace([folderA, folderB], "b")
