...
export function OutputPanel({ workspace }: { workspace: WorkspaceService }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-jet-list-panel="output">
      <div className="shrink-0 border-b border-[var(--jet-border)] px-2 py-1 text-xs uppercase tracking-wide text-[var(--jet-text-muted)]">
        Output {run ? `— ${run.task.label} (${run.status})` : ""}
      </div>
      <pre
        ref={logRef}
        className="min-h-0 flex-1 overflow-auto p-2 font-mono text-xs leading-relaxed text-[var(--jet-text)]"
      >
        {run?.output ?? "No task output. Run a task from the command palette."}
      </pre>
      {run && run.errors.length > 0 && (
        <div className="shrink-0 border-t border-[var(--jet-border)] p-1 text-xs text-[var(--jet-error)]">
