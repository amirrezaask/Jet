...
function readStat(file: AgentFileChange): TurnDiffStat {
  return {
...
export function summarizeTurnDiffStats(files: ReadonlyArray<AgentFileChange>): TurnDiffStat {
  return files.reduce(
    (acc, file) => {
...
export function buildTurnDiffTree(files: ReadonlyArray<AgentFileChange>): TurnDiffTreeNode[] {
