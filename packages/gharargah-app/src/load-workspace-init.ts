...
/** Handles passed to `.gharargah/editorrc.ts` — same registries Gharargah uses internally. Import `bind` from `@gharargah/workspace` in init code. */
...
export async function loadWorkspaceInit(
  ...
  const fs = typeof window !== "undefined" ? window.gharargah?.fs : undefined
