...
import type { PanelId, Edge } from "@gharargah/shared"
...
import { folderForFileUri, resolveWorkspaceFolder } from "@gharargah/workspace"
import { PROBLEMS_TAB_ID, panelTabIds } from "@gharargah/workspace"
import { problemsToListItems } from "@gharargah/ui"
...
import { scheduleCodeActions, applyCodeAction } from "@gharargah/lsp"
import type { OutlineEntry } from "@gharargah/ui"
import { getEditorView, showEditorContextMenuAt, destroyEditorBuffer, lspLocationsToListItems } from "@gharargah/ui"
import {
...
import type { JetSidebarView } from "@gharargah/ui"
import {
  anyOverlayOpen,
  bind,
  type JetKeyBinding,
...
export function buildAppCommands(deps: BuildAppCommandsDeps): JetCommands {
  ...
  const named: JetCommands = {
    ...
    runTask: async ctx => {
      ...
      if (!tasks.length) {
        ctx.ui.showMessage("No tasks — add .gharargah/tasks.json or register in editorrc")
        return
      }
      const task = tasks[0]!
...
export const APP_COMMAND_REGISTRY = [
  ...
  { id: "gharargah.openEditor", fn: "openEditorShell", title: "Open Editor", category: "View", aliases: ["workspace", "editor shell"] },
