  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          id={`yaade-session-tab-${mode}`}
          aria-label={label}
          aria-controls={`yaade-session-pane-${mode}`}
          aria-pressed={active}
          title={title}
          disabled={disabled}
          data-yaade-session-mode-tab={mode}
          data-yaade-session-mode-agent={
            mode === "agent" && agentId ? agentId : undefined
          }
          data-active={active ? "" : undefined}
          onClick={onSelect}
          className={cn(
            "relative flex size-9 items-center justify-center rounded-[0.7rem] border text-muted-foreground outline-none transition-[color,background-color,border-color,box-shadow,transform] duration-[var(--yaade-motion-fast)] ease-[var(--yaade-ease-out)] focus-visible:ring-2 focus-visible:ring-ring/50 active:scale-[var(--yaade-press-scale)] disabled:pointer-events-none disabled:opacity-40",
            active
