...
export function createProjectTodosRepository(
  ...
  const notify = () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(new Event("yaade:project-todos"))
    }
  }
 ...
 * Browser: ONLY `window.__yaadeProjectTodos` (survives dual Vite chunks / HMR).
...
export function getSharedRepository(): ProjectTodosRepository {
  if (typeof window !== "undefined") {
    if (!window.__yaadeProjectTodos) {
      window.__yaadeProjectTodos = createProjectTodosRepository(defaultStorage())
    }
    return window.__yaadeProjectTodos
  }
  if (!nodeRepo) {
...
declare global {
  interface Window {
    __yaadeProjectTodos?: ProjectTodosRepository
  }
}
