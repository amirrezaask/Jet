...
export function ProjectTodoProgress(props: ProjectTodoProgressProps) {
  ...
  if (variant === "linear") {
    return (
      ...
      >
        <div
          className="h-full rounded-full bg-primary transition-[width] duration-[var(--yaade-motion-fast)] ease-[var(--yaade-ease-out)] motion-reduce:transition-none"
          style={{ width: `${pct}%` }}
        />
      </div>
    )
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        strokeLinecap="round"
        className="transition-[stroke-dashoffset] duration-[var(--yaade-motion-fast)] ease-[var(--yaade-ease-out)] motion-reduce:transition-none"
      />
    </svg>
  )
}
