...
import { basename, fileUriToPath, isUntitledUri, pathToFileUri } from "@yaade/shared"
import type {
  JetCommandContext,
  JetCommands,
  JetCommandFn,
...
import { getEditorView, destroyEditorBuffer } from "@yaade/ui"
import {
...
async function withMonacoApi<T>(
  run: (api: typeof import("@yaade/monaco")) => T | Promise<T>,
): Promise<T | undefined> {
  const api = await import("@yaade/monaco")
  return run(api)
}

export type BuildAppCommandsDeps = {
  workspace: WorkspaceService
  getPanelTree: () => YaadePanelTree
  ...
  cloneTree: () => YaadePanelTree
  commitTree: (tree: YaadePanelTree, preferFocus?: PanelId | null) => void
  ...
  projectRegistry: import("@yaade/workspace").ProjectRegistry
  ...
  getContextFolder: () => import("@yaade/workspace").WorkspaceFolder | null
...
export function buildAppCommands(deps: BuildAppCommandsDeps): JetCommands {
  ...
  async function gitSearchUnavailable(ctx: JetCommandContext): Promise<boolean> {
    ...
    try {
      if (await window.yaade?.search?.isSupported?.(folder.root.uri)) return false
    } catch {
      /* fall through */
    }
  ...
  const named: JetCommands = {
    ...
    save: async ctx => {
      ...
      const fileUri = panel && getActiveEditorFileUri(currentPanelTree(), panel)
      if (!fileUri) return
      const { getEditorContent } = await import("@yaade/monaco")
      const content = getEditorContent(editor) ?? ""
      if (isUntitledUri(fileUri)) {
        const savePath = (await window.yaade?.fs.showSaveFileDialog()) ?? null
        if (!savePath) return
...
export const APP_COMMAND_REGISTRY = [
  ...
  { id: "yaade.goHome", fn: "goHome", title: "Go Home", category: "View", aliases: ["mission control", "home"] },
