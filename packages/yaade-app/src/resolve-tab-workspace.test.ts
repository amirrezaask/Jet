...
import { YaadePanelTree, buildTabsView } from "@yaade/workspace"
import type { WorkspaceFolder } from "@yaade/workspace"
import {
  isContextualTabKind,
  ...
  return {
    ...
    tabRegistry: {
      kindFor(id: string) {
        ...
        if (id.startsWith("yaade:terminal:")) return "terminal"
        if (id === "yaade:search") return "search"
        return undefined
      },
    },
...
describe("resolveContextWorkspaceFolder", () => {
  it("uses last context when search tab is focused", () => {
    const { tree, editorPanel } = YaadePanelTree.editorOnlyLayout()
    tree.setView(editorPanel, buildTabsView("yaade:search", ["yaade:search"]))
    const workspace = mockWorkspace([folderA, folderB], "b")
