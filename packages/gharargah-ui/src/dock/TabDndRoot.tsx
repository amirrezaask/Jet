...
import type { DropAction, PanelId } from "@gharargah/shared"
import { GharargahTabDragGhost } from "@/motion/GharargahOverlayMotion.js"
...
function resolveDropAnimTarget(hot: DropHotState): DropAnimTarget | null {
  ...
  const overlay = document.querySelector<HTMLElement>(
    `[data-gharargah-panel-drop-overlay][data-gharargah-drop-panel="${hot.panelId.id}"]`,
  )
...
function TabDndInner({ children, handlers }: TabDndInnerProps) {
  ...
  const onDragStart = useCallback(
    (event: DragStartEvent) => {
      ...
      const els = document.querySelectorAll<HTMLElement>("[data-gharargah-panel-drop-overlay]")
      const snapshots: OverlaySnapshot[] = []
      for (const el of els) {
        if (el.closest("[data-gharargah-layout-morph-clone]")) continue
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={dropAnimation}>
        {activeTab ? (
          <GharargahTabDragGhost label={activeTab.label} dirty={activeTab.dirty} />
        ) : null}
      </DragOverlay>
    </DndContext>
  )
