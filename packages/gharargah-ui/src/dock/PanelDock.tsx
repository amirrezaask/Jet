...
import type { PanelEvent, PanelNode } from "@gharargah/panels"
import type { PanelTree } from "@gharargah/panels"
...
function splitPanelDomId(path: number[], index: number): string {
  return path.length === 0 ? `gharargah-split-${index}` : `gharargah-split-${path.join(".")}-${index}`
}

function splitGroupDomId(path: number[]): string {
  return path.length === 0 ? "gharargah-root-split" : `gharargah-split-group-${path.join(".")}`
}
...
function PanelLeaf<TView>({
  ...
  return (
    <div
      ...
      data-gharargah-panel-leaf={panelId.id}
      data-gharargah-panel-dragged-over={dragOver && isDropTarget ? "" : undefined}
      ...
      {children.map((child, index) => (
        <Fragment key={splitPanelDomId(path, index)}>
          {index > 0 ? (
            <ResizableHandle
              withHandle
              className="transition-colors duration-[var(--gharargah-motion-fast)] hover:bg-primary/20"
            />
          ) : null}
          <ResizablePanel
...
function PanelDockInner<TView>(props: PanelDockProps<TView>) {
  return (
    <TabDndRoot handlers={props.tabDnd}>
      <div className="flex h-full min-h-0 w-full flex-col overflow-hidden" data-gharargah-panel-dock>
