...
function describeUnavailableInstance(entry: ProviderInstanceEntry): string {
  ...
  const kind =
    entry.status === "unavailable" ? "Unavailable" : entry.status === "pending" ? "Not ready" : "Limited"
  const msg = entry.snapshot.message?.trim()
  return msg ? `${label} — ${kind}. ${msg}` : `${label} — ${kind}.`
}

const SELECTED_INDICATOR_CLASS =
  "pointer-events-none absolute -right-1 top-0 z-10 h-5 w-0.75 rounded-l-full bg-primary";
          ...
          {selectedIndicatorTop !== null ? (
            <div
              ...
              className={cn(
                ...
                "right-0 transition-transform duration-[var(--gharargah-motion-menu)] ease-[var(--gharargah-ease-out)]",
              )}
              style={{ transform: `translate3d(0, ${selectedIndicatorTop}px, 0)` }}
            />
          ) : null}
          {/* Favorites section */}
          {showFavorites ? (
            ...
            const button = (
              ...
              >
                <ProviderInstanceIcon
                  ...
                  {...(entry.accentColor
                    ? { badgeClassName: "h-3 min-w-3 px-0.5 text-4xs" }
                    : {})}
                />
                {showNewBadge ? (
