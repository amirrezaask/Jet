...
function createStreakLayer(): StreakEntry {
  ...
  stopA.setAttribute("stop-color", "var(--gharargah-cursor-color, #c4923a)")
  const stopB = document.createElementNS(SVG_NS, "stop")
  stopB.setAttribute("offset", "1")
  stopB.setAttribute("stop-color", "var(--gharargah-cursor-color, #c4923a)")
...
export function useGharargahCaretOverlay(
  ...
  useEffect(() => {
    ...
    if (document.querySelector("[data-gharargah-universal-caret]")) return
    ...
    caretBar.style.background = "var(--gharargah-cursor-color, #c4923a)"
...
export const GharargahCaretInput = forwardRef<
  HTMLInputElement,
  InputHTMLAttributes<HTMLInputElement> & { caretOverlay?: boolean }
>(function GharargahCaretInput({ className, caretOverlay = true, ...props }, ref) {
  const innerRef = useRef<HTMLInputElement>(null)
  const anchorRef = useRef<HTMLDivElement>(null)
  useGharargahCaretOverlay(innerRef, caretOverlay, anchorRef)

  return (
    <div ref={anchorRef} data-gharargah-caret-anchor="" className="relative min-w-0">
      <input
        ref={el => {
          innerRef.current = el
...
export function useGharargahCaretOverlayRef<T extends HTMLInputElement>(
  externalRef: RefObject<T | null>,
  enabled = true,
): RefObject<T | null> {
  useGharargahCaretOverlay(externalRef, enabled)
  return externalRef
}
