...
import type { WorkspaceEntry, WorkspaceManager } from "@gharargah/workspace"
        ...
        if (node.data.kind === "root") {
          return (
            <>
              <span
                aria-hidden
                data-gharargah-project-activity={node.data.uri === activeRootId ? "active" : "idle"}
                className={node.data.uri === activeRootId ? "h-4 w-0.5 rounded-full bg-primary" : "h-1.5 w-0.5 rounded-full bg-muted-foreground/35"}
              />
              <Folder data-gharargah-project-icon className="size-5! shrink-0 text-foreground/85" />
              <span
