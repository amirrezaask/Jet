...
import { lspLanguageIdFromJet } from "@gharargah/shared"
import type { WorkspaceFile } from "@gharargah/workspace"
import type { WorkspaceService } from "@gharargah/workspace"
import type { JetKeyBinding } from "@gharargah/workspace"
import type { KeymapContext } from "@gharargah/workspace"
import { jetKeyToCodeMirrorKey, matchesWhen, isEditorKeyBinding } from "@gharargah/workspace"
import { jetEditorTheme, jetSyntaxHighlightingForTheme } from "./theme.js"
import { defaultGharargahTheme, type GharargahTheme } from "./theme-types.js"
...
/** Sublime-style multi-cursor keys at CM layer (Gharargah keymap also binds these for palette/chords). */
...
function indentMarkerExtension(theme: GharargahTheme, largeFile: boolean): Extension {
...
export type CreateGharargahEditorViewOptions = {
  parent: HTMLElement
...
export async function createGharargahEditorView(opts: CreateGharargahEditorViewOptions): Promise<EditorView> {
  const theme = opts.theme ?? defaultGharargahTheme
  const largeFile = opts.largeFile ?? false
  const indent = opts.indent ?? detectIndent(opts.initialText)

  /** OS file drops on the editor open the file in Gharargah; never insert file contents into the buffer. */
...
export function applyTheme(view: EditorView, theme: GharargahTheme): void {
...
export function applyUserKeymaps(
  ...
  perfMeasure("gharargah:apply-user-keymaps", () => {
