...
async function loadJetrcJson(jetDir: string): Promise<string[] | null> {
  try {
    const text = await fs.readFile(path.join(jetDir, "jetrc.json"), "utf8")
...
/** Load project scan roots from `~/.jet/jetrc.json` or `~/.jet/jetrc.ts`. */
export async function loadGlobalJetrcScanRoots(homeDir: string): Promise<string[]> {
  const jetDir = path.join(homeDir, ".jet")
