...
pub fn run() {
    ...
    #[cfg(feature = "e2e")]
    {
        builder = builder
            .append_invoke_initialization_script(
                r#"if (window.name !== "__gharargah_e2e_initialized__") {
                    localStorage.clear();
                    sessionStorage.clear();
                    window.name = "__gharargah_e2e_initialized__";
                }"#,
            )
    ...
    let app = builder
        .setup(move |app| {
            ...
            let e2e = std::env::var("GHARARGAH_E2E").ok().as_deref() == Some("1");
            let headed = std::env::var("GHARARGAH_HEADED").ok().as_deref() == Some("1")
        ...
        .invoke_handler(tauri::generate_handler![
            shell::gharargah_show_open_folder_dialog,
            shell::gharargah_show_save_file_dialog,
            shell::gharargah_sync_native_chrome,
            shell::gharargah_get_launch_config,
            shell::gharargah_host_invoke,
        ])
        .build(tauri::generate_context!())
        .expect("error while building gharargah");
