...
#[tauri::command]
pub async fn gharargah_show_open_folder_dialog(window: WebviewWindow) -> Result<Option<String>, String> {
...
#[tauri::command]
pub async fn gharargah_show_save_file_dialog(
...
#[tauri::command]
pub async fn gharargah_sync_native_chrome(
    window: WebviewWindow,
...
#[tauri::command]
pub fn gharargah_get_launch_config(state: tauri::State<ShellState>) -> Option<LaunchConfig> {
...
#[tauri::command]
pub async fn gharargah_host_invoke(
    app: AppHandle,
    window: WebviewWindow,
    ...
    if channel == "gharargah:getLaunchConfig" {
        return Ok(shell
            .launch_config
            .lock()
            .ok()
    ...
    if channel == "ui:syncNativeChrome" {
        if let Some(colors) = args.first() {
            if let Ok(parsed) = serde_json::from_value::<NativeChromeColors>(colors.clone()) {
                gharargah_sync_native_chrome(window, parsed).await?;
            }
        }
        return Ok(Value::Null);
    }
    if channel == "fs:showOpenFolderDialog" {
        return Ok(serde_json::to_value(gharargah_show_open_folder_dialog(window).await?).unwrap());
    }
    if channel == "fs:showSaveFileDialog" {
        let default_path = args.first().and_then(|v| v.as_str()).map(str::to_string);
        return Ok(
            serde_json::to_value(gharargah_show_save_file_dialog(window, default_path).await?).unwrap(),
        );
    }
...
pub fn install_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    #[cfg(target_os = "macos")]
    let app_submenu = {
        let about = PredefinedMenuItem::about(app, Some("About Gharargah"), None)?;
...
pub fn on_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    ...
    match id {
        "close-tab" => emit_host(app, "gharargah:close-tab", vec![]),
...
fn chrome_cache_path() -> Option<PathBuf> {
    ...
    Some(home.join(".gharargah").join("native-chrome.json"))
}
