...
const binary = path.join(target, process.platform === "win32" ? "gharargah.exe" : "gharargah")
const sample = path.join(repo, "fixtures", "sample-workspace")
const perfDir = path.join(os.homedir(), ".gharargah", "perf")
...
async function waitForRecord(sampleIndex, timeoutMs = 30_000) {
  ...
  throw new Error(`startup sample ${sampleIndex} did not reach Gharargah ready in ${timeoutMs}ms`)
}
...
for (let sampleIndex = 0; sampleIndex < rounds; sampleIndex++) {
  const child = spawn(binary, [sample], {
    ...
    env: {
      ...process.env,
      GHARARGAH_STARTUP_RUN_ID: runId,
      GHARARGAH_STARTUP_SAMPLE: String(sampleIndex),
      GHARARGAH_STARTUP_RUN_KIND: sampleIndex === 0 ? "cold" : "warm",
      GHARARGAH_BUILD_COMMIT: commit,
    },
  })
  try {
