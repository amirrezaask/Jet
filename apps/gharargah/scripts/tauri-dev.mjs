...
const devPort = Number(process.env.GHARARGAH_TAURI_DEV_PORT ?? 5174)
...
if (launchArgs.length > 0) {
  process.env.GHARARGAH_LAUNCH_ARGS = JSON.stringify(launchArgs)
}

await waitForPortFree(devPort)
process.env.GHARARGAH_TAURI_DEV_PORT = String(devPort)
