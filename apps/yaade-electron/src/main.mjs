...
const yaadeAppDir = path.resolve(repoRoot, "apps/yaade")
...
const isDev = process.argv.includes("--dev") || process.env.YAADE_ELECTRON_DEV === "1"
...
function resolveRuntimeRoot() {
  if (app.isPackaged) {
    ...
    const legacy = path.join(process.resourcesPath, "yaade")
    if (isPackagedRuntime(legacy)) return legacy
  }
  // Local unpack smoke: YAADE_RUNTIME_ROOT=/path/to/pack
  const fromEnv = process.env.YAADE_RUNTIME_ROOT
...
async function ensureLocalBackends() {
  ...
  if (isDev && !viteUp) {
    children.push(spawnVite({ appDir: yaadeAppDir }))
  }
...
async function waitForYaadeServer(serverUrl, timeoutMs) {
...
async function selectInitialServer(configPath) {
  ...
  if (selectedServerUrl) {
    try {
      await waitForYaadeServer(selectedServerUrl, 5_000)
      activeServerUrl = selectedServerUrl
      return selectedServerUrl
    } catch (error) {
      startupError = error instanceof Error ? error.message : String(error)
      console.error(`[yaade-electron] ${startupError}; falling back to bundled server`)
    }
  }
...
function registerServerIpc(configPath) {
  ipcMain.handle("yaade:server:get", () => serverConnection())
  ipcMain.handle("yaade:server:connect", async (_event, requestedUrl) => {
    const remoteUrl =
    ...
    if (remoteUrl) {
      await waitForYaadeServer(remoteUrl, 10_000)
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error("[yaade-electron] backend startup failed:", err)
    stopChildren()
    app.exit(1)
    return
  }
