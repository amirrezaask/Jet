...
pub fn load_global_jetrc_scan_roots(home_dir: &str) -> Vec<String> {
    let jet_dir = Path::new(home_dir).join(".jet");
    let json_path = jet_dir.join("jetrc.json");
...
pub fn handle(channel: &str, _args: &[Value], home_dir: &str) -> Result<Value, String> {
    match channel {
        "jet:getHomeDir" => Ok(Value::String(home_dir.to_string())),
        "jet:loadGlobalJetrcScanRoots" => Ok(serde_json::to_value(load_global_jetrc_scan_roots(home_dir))
