...
impl PerfHost {
    pub fn new(home_dir: &str, process_started: Instant) -> Self {
        let base = std::env::var_os("JET_E2E_USER_DATA")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from(home_dir).join(".jet"));
        Self {
            process_started,
    ...
    pub fn record_startup(&self, payload: &Value) -> Result<Value, String> {
        ...
        object.insert(
            "buildMode".into(),
            Value::String(if cfg!(debug_assertions) { "debug" } else { "release" }.into()),
        );
        for (field, env_name) in [
            ("runId", "JET_STARTUP_RUN_ID"),
            ("runKind", "JET_STARTUP_RUN_KIND"),
            ("commit", "JET_BUILD_COMMIT"),
            ("sample", "JET_STARTUP_SAMPLE"),
        ] {
