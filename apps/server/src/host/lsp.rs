...
impl LspHost {
    ...
    pub fn start(
        ...
        let id = format!("lsp-{}-{}", cmd_name, chrono::Utc::now().timestamp_millis());
        ...
        Ok(serde_json::json!({
            "id": id,
            "transportUrl": format!("/ws/lsp/{id}")
        }))
    }

    pub fn transport_port(&self, id: &str) -> Option<u16> {
        self.sessions
            .lock()
            .ok()?
            .get(id)
...
mod tests {
    ...
    fn resolve_command_path_finds_executable_on_path() {
        let dir =
            std::env::temp_dir().join(format!("gharargah-lsp-resolve-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        let bin = dir.join("gharargah-fake-lsp");
        ...
        let resolved = resolve_command_path("gharargah-fake-lsp").unwrap();
    ...
    fn websocket_reconnect_keeps_child_alive() {
        ...
        ws1.send(Message::Text(framed)).unwrap();
        thread::sleep(Duration::from_millis(50));
        ...
        ws2.send(Message::Text(framed2)).unwrap();
        let _ = ws2.read();
