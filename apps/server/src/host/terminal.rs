...
impl TerminalHost {
    ...
    fn spawn_one(
        ...
        thread::spawn(move || {
            ...
            let exit_code = status.as_ref().map(|s| s.exit_code() as i32).unwrap_or(1);
    ...
    pub fn attach(&self, client_id: &str, id: &str) -> Result<Option<Value>, String> {
        ...
        let mut e = entry.lock().map_err(|e| e.to_string())?;
        // Browser refreshes create a new transport id. The server owns the PTY,
        // so attaching transfers observation without terminating the process.
...
fn primary_shell() -> ShellSpec {
    if cfg!(windows) {
        ...
        return ShellSpec { file, args: vec![] };
    }
...
mod tests {
    ...
    fn replay_trim_preserves_utf8_boundaries_and_tail() {
        ...
        push_terminal_replay(
            &mut output,
            &mut output_bytes,
            format!("{}gharargah-unicode-tail", "سلام🙂".repeat(128)),
            73,
        );
        let joined = output.iter().map(String::as_str).collect::<String>();
        assert!(output_bytes <= 73);
        assert!(joined.ends_with("gharargah-unicode-tail"));
