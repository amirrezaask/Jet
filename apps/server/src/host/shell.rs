...
pub fn open_in_app(app_id: &str, root_uri: &str) -> Result<(), String> {
    ...
    match app_id {
        "vscode" => try_cmds(&[&["code", "-n", &path], &["code.cmd", "-n", &path]])
            .or_else(|_| open_macos_app("Visual Studio Code", &path)),
        "cursor" => try_cmds(&[&["cursor", &path], &["cursor.cmd", &path]])
            .or_else(|_| open_macos_app("Cursor", &path)),
        "emacs" => try_cmds(&[&["emacs", &path], &["emacsclient", "-n", "-a", "", &path]])
            .or_else(|_| open_macos_app("Emacs", &path)),
        "sublime" => try_cmds(&[&["subl", &path], &["subl.exe", &path]])
            .or_else(|_| open_macos_app("Sublime Text", &path)),
        "zed" => try_cmds(&[&["zed", &path], &["zed.exe", &path]])
        ...
        "xcode" => try_cmds(&[&["xed", &path]]).or_else(|_| open_macos_app("Xcode", &path)),
        "intellij" => try_cmds(&[
            &["idea", &path],
            &["idea64", &path],
