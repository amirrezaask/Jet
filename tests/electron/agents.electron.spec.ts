...
describeFlaky("electron agents", () => {
  ...
  test("real cursor-agent turn completes in agent chat", async () => {
    ...
    try {
      ...
      const chatTab = page.locator('[data-gharargah-tab-slot^="gharargah:agent-chat:"][data-gharargah-tab-active]')
      ...
      const threadStatus = await page.evaluate(async () => {
        const folder = window.__gharargahAgent!.getState().workspacePath
        if (!folder || !window.gharargah?.agents?.readThread) return null
        const snapshot = await window.gharargah.agents.listThreads(`file://${folder}`, folder)
        const threadId = snapshot.threads[0]?.id
        if (!threadId) return null
        const thread = await window.gharargah.agents.readThread(`file://${folder}`, folder, threadId)
